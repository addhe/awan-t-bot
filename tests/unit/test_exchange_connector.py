...
def mock_ccxt():
    ...
    with patch('src.exchange.connector.ccxt') as mock_ccxt:
        ...
        mock_exchange.fetch_ohlcv.return_value = [
            [1625097600000, 35000, 36000, 34500, 35500, 10.5],  # timestamp, open, high, low, close, volume
...
class TestExchangeConnector:
    ...
    async def test_fetch_ohlcv(self, exchange_connector, mock_ccxt):
        ...
        # Call the method
        df = await exchange_connector.fetch_ohlcv('BTC/USDT', timeframe='1h', limit=10)
        
        # Check if ccxt method was called correctly
        mock_exchange.fetch_ohlcv.assert_called_once_with('BTC/USDT', '1h', limit=10)
    ...
    async def test_place_market_sell(self, exchange_connector, mock_ccxt):
        ...
        # Check if ccxt method was called correctly
        mock_exchange.create_market_sell_order.assert_called_once_with('BTC/USDT', 0.01)
