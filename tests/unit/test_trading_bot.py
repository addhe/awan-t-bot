...
def mock_strategy():
    ...
    mock.analyze_signals.return_value = ("neutral", 0.0, {"stop_loss": 0, "take_profit": 0})
    mock.should_sell.return_value = (False, 0.0)
    mock.calculate_position_size.return_value = (0.01, {
        'allocation_pct': 20,
        'allocation_usdt': 100,
        'max_allocation': 100
    })
...
@pytest.fixture
def trading_bot(monkeypatch, mock_exchange, mock_strategy, mock_position_manager, mock_monitor):
    """Create a TradingBot with mock dependencies"""
    # Setup patches
    monkeypatch.setattr('src.core.trading_bot.ExchangeConnector', lambda *args: mock_exchange)
    monkeypatch.setattr('src.core.trading_bot.BollStochStrategy', lambda **kwargs: mock_strategy)
    monkeypatch.setattr('src.core.trading_bot.PositionManager', lambda *args: mock_position_manager)
    monkeypatch.setattr('src.core.trading_bot.send_telegram_message', AsyncMock())
    
    # Create bot instance and inject mock monitor
    bot = TradingBot()
    bot.monitor = mock_monitor
    
    return bot, mock_exchange, mock_strategy, mock_position_manager, mock_monitor
...
class TestTradingBot:
    ...
    async def test_initialize(self, trading_bot):
        ...
        bot, mock_exchange, mock_strategy, mock_position_manager, _ = trading_bot
    ...
    async def test_process_pair_no_signal(self, trading_bot):
        ...
        mock_strategy.analyze_signals.return_value = ("neutral", 0.0, {"stop_loss": 0, "take_profit": 0})
        
        # Call the method
        pair_config = {'symbol': 'BTC/USDT', 'min_quantity': 0.001, 'quantity_precision': 5}
    ...
    async def test_process_pair_buy_signal(self, trading_bot):
        ...
        mock_strategy.analyze_signals.return_value = ("buy", 0.8, {"stop_loss": 33000, "take_profit": 38000})
    ...
    async def test_calculate_performance(self, trading_bot):
        ...
        assert performance['win_rate'] == pytest.approx(66.67, abs=0.01)  # 2/3 winning trades
