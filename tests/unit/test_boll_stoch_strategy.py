...
class TestBollStochStrategy:
    ...
    def test_analyze_signals_buy(self, strategy, sample_df):
        ...
        result_df.iloc[-1, result_df.columns.get_loc('close')] = result_df['bb_lower'].iloc[-1] * 0.99
        result_df.iloc[-1, result_df.columns.get_loc('ema')] = result_df['close'].iloc[-1] * 0.98
        ...
        signal, confidence, risk_levels = strategy.analyze_signals(timeframe_data)
    ...
    def test_analyze_signals_sell(self, strategy, sample_df):
        ...
        result_df.iloc[-1, result_df.columns.get_loc('close')] = result_df['bb_upper'].iloc[-1] * 1.01
        result_df.iloc[-1, result_df.columns.get_loc('ema')] = result_df['close'].iloc[-1] * 1.02
    ...
    def test_calculate_position_size(self, strategy):
        ...
        # Validate result
        expected_allocation = min(balance * trading_config['allocation_per_trade'], 
                                 trading_config['max_allocation_usdt'])
        expected_quantity = expected_allocation / current_price
        
        assert quantity == round(expected_quantity, pair_config['quantity_precision'])
        assert allocation_info['allocation_pct'] == trading_config['allocation_per_trade'] * 100
