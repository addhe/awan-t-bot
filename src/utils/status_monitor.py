...
class BotStatusMonitor:
    ...
    def _ensure_status_dir(self):
        ...
        try:
            ...
            logger.debug(f"Ensured status directory exists: {self.status_dir}")
        except Exception as e:
            logger.error(f"Failed to create status directory: {self.status_dir}", exc_info=True)
            raise FileOperationError(f"Failed to create status directory: {self.status_dir}", e)
    ...
    def update_bot_status(self, status: Dict[str, Any]):
        ...
        try:
            ...
            error_msg = f"Error updating bot status"
            logger.error(error_msg, exc_info=True, status_file=self.status_file)
            raise FileOperationError(error_msg, e, {"status_file": self.status_file})
    ...
    def update_trades(self, trades: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.info(f"Active trades updated successfully", trade_count=len(trades))
        except Exception as e:
            error_msg = "Error updating trades status"
            logger.error(error_msg, exc_info=True, trades_file=self.trades_file, trade_count=len(trades))
            raise FileOperationError(error_msg, e, {"trades_file": self.trades_file})
        ...
        try:
            if os.path.exists(self.status_file):
                with open(self.status_file, 'r') as f:
                    status = json.load(f)
                    logger.debug("Retrieved bot status successfully")
                    return status
            logger.info("No existing status file found, returning empty status")
            return {}
        ...
        try:
            if os.path.exists(self.trades_file):
                with open(self.trades_file, 'r') as f:
                    ...
                    logger.debug(f"Retrieved active trades successfully", trade_count=len(trades))
                    return trades
            logger.info("No existing trades file found, returning empty list")
            return []
        except json.JSONDecodeError as e:
            error_msg = "Error decoding active trades JSON"
            logger.error(error_msg, exc_info=True, trades_file=self.trades_file)
            raise FileOperationError(error_msg, e, {"trades_file": self.trades_file})
        except Exception as e:
        ...
        try:
            if os.path.exists(self.completed_trades_file):
                with open(self.completed_trades_file, 'r') as f:
                    ...
                    logger.debug(f"Retrieved completed trades successfully", 
                                trade_count=len(trades), 
                                since=since.isoformat() if since else None)
                    return trades
            logger.info("No existing completed trades file found, returning empty list")
            return []
        except json.JSONDecodeError as e:
            ...
            raise FileOperationError(error_msg, e, {"completed_trades_file": self.completed_trades_file})
        except Exception as e:
        ...
        try:
            ...
            if os.path.exists(self.completed_trades_file):
                with open(self.completed_trades_file, 'r') as f:
                    ...
                    logger.debug(f"Loaded existing completed trades", count=len(completed_trades))
            ...
            logger.info(f"Completed trade saved successfully", 
            ...
            logger.error(error_msg, exc_info=True, 
                        ...
                        symbol=trade.get('symbol'))
            raise FileOperationError(error_msg, e, {
                "completed_trades_file": self.completed_trades_file,
                "symbol": trade.get('symbol')
            })
        except Exception as e:
    ...
    def format_status_message(self) -> str:
        ...
        try:
            ...
            logger.debug("Retrieved data for status message", 
            ...
            logger.debug("Status message formatted successfully", msg_length=len(msg))
            return msg
            
        except Exception as e:
            logger.error("Error formatting status message", exc_info=True)
