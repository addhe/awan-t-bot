...
def place_order_with_sl_tp(
    ...
    try:
        ...
        tp2_price = price * (1 + CONFIG['tp2_target']) if side == 'buy' else price * (1 - CONFIG['tp2_target'])
        ...
        logging.info(f"Orders placed - Main: {main_order['id']}, SL: {sl_order['id']}, TP1: {tp1_order['id']}, TP2: {tp2_order['id']}")
...
def close_position(
    ...
    try:
        ...
        for position in positions:
            if position['side'] == position_side and float(position['contracts']) > 0:
                ...
                logging.info(f"Closed {position_side} position for {symbol}")
                return True
