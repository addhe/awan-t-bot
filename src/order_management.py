...
def place_order_with_sl_tp(
    ...
    try:
        ...
        main_order = exchange.create_order(
            symbol=exchange.symbol, type="market", side=side, amount=amount
        )
...
def check_funding_rate(
    exchange: ccxt.Exchange, intended_side: Optional[str] = None
) -> Dict[str, Any]:
    try:
        # Fetch funding rate
        ...
        if intended_side == "buy":
            is_favorable = (
                current_rate < -0.01
            )  # Favorable for longs if negative
        elif intended_side == "sell":
            is_favorable = (
                current_rate > 0.01
            )  # Favorable for shorts if positive

        return {
            "current_rate": current_rate,
            "next_funding_time": next_funding_time,
            "is_favorable": is_favorable,
            "should_wait": abs(current_rate)
            > 0.05,  # Wait if funding rate is extreme
        }
    except Exception as e:
...
def handle_order_error(
    e: Exception, side: str, amount: float
) -> Dict[str, Any]:
    error_info = {
...
def close_position(
    exchange: ccxt.Exchange, position: Dict[str, Any]
) -> Dict[str, Any]:
    try:
        # Close the position
        ...
        return {"success": False, "error": str(e), "position": position}
